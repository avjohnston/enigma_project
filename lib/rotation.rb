...
class Rotation
  ...
  def letter_1_shift(letter, type, key = @key, date = @date)
    unless !@chars.include?(letter)
      old_letter = @chars.rotate(@chars.index(letter))
      new_letter = old_letter.rotate((date.a_rotation + key.a_rotation) % 27).first if type == :encryption
      new_letter = old_letter.rotate(- ((date.a_rotation + key.a_rotation) % 27)).first if type == :decryption
      new_letter
    else
      new_letter = letter
    end
  ...
  def apply_shift(message, type, key = @key, date = @date)
    ...
    letters.each do |letter|
      ...
      new_text += letter_1_shift(letter, type, key, date) if letter_number == 1
      new_text += letter_2_shift(letter, type, key, date) if letter_number == 2
      new_text += letter_3_shift(letter, type, key, date) if letter_number == 3
      new_text += letter_4_shift(letter, type, key, date) if letter_number == 4
    end
    new_text
  end
end
